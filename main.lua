...
local MainFrame = Instance.new("Frame", ScreenGui); MainFrame.Size = UDim2.new(0, 600, 0, 400); MainFrame.Position = UDim2.new(0.5, 0, 0.5, 0); MainFrame.AnchorPoint = Vector2.new(0.5, 0.5); MainFrame.BackgroundColor3 = Settings.Theme.Dark; MainFrame.Visible = false; MainFrame.ClipsDescendants = true; Library:Corner(MainFrame, 12); Library:AddGlow(MainFrame)
...
local KeyInput = Instance.new("TextBox", KeyFrame); KeyInput.Size = UDim2.new(0.7,0,0,45); KeyInput.Position = UDim2.new(0.15,0,0.4,0); KeyInput.BackgroundColor3 = Settings.Theme.Box; KeyInput.TextColor3 = Color3.new(1,1,1); KeyInput.PlaceholderText = "Enter Key... / הכנס מפתח"; KeyInput.Text = ""; KeyInput.Font = Enum.Font.Gotham; KeyInput.TextSize = 14; Library:Corner(KeyInput, 6); Library:AddGlow(KeyInput, Color3.fromRGB(60,60,60))
local KeyBtn = Instance.new("TextButton", KeyFrame); KeyBtn.Size = UDim2.new(0.4,0,0,40); KeyBtn.Position = UDim2.new(0.3,0,0.75,0); KeyBtn.BackgroundColor3 = Settings.Theme.Gold; KeyBtn.Text = "LOGIN / כניסה"; KeyBtn.Font = Enum.Font.GothamBold; KeyBtn.TextSize = 16; Library:Corner(KeyBtn, 6)

KeyBtn.MouseButton1Click:Connect(function()
    if KeyInput.Text == Settings.Key then
        Library:Tween(KeyFrame, {Size = UDim2.new(0,0,0,0), Position = UDim2.new(0.5,0,0.5,0)}, 0.4, Enum.EasingStyle.Back); task.wait(0.3); KeyFrame.Visible = false; MainFrame.Visible = true; MainFrame.Size = UDim2.new(0,0,0,0); Library:Tween(MainFrame, {Size = UDim2.new(0, 600, 0, 400)}, 0.6, Enum.EasingStyle.Elastic)
    end
end)
