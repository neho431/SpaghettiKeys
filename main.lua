--[[
    Spaghetti Mafia Hub v1 (FINAL STABLE - SIMPLE KEY SYSTEM)
    Branding: "עולם הכיף"
    Fixes: Removed API Blocks, Fixed Syntax, Merged StartHub, Fixed Corners.
]]
...
--// ניקוי סקריפטים ישנים למניעת כפילויות על המסך
...
--// הגדרות GitHub (אימות פשוט מול קובץ טקסט)
...
--// פונקציית ה-Hub (תופעל רק אחרי אימות מוצלח)
function StartHub()
    local Settings = {
    ...
    local VisualToggles, FarmConnection, FarmBlacklist = {}, nil, {}
    ...
    MainFrame.Visible = true -- מופעל כעת כי המפתח אושר
    Library:Corner(MainFrame, 12)
    Library:AddGlow(MainFrame)
    
    -- אנימציית פתיחה ל-Hub
    ...
    MainTitle.Position = UDim2.new(0,50,0,10)
    MainTitle.BackgroundTransparency = 1
    MainTitle.Text = "SPAGHETTI <font color='#FFD700'>MAFIA</font> HUB v1"
    ...
    local function CreateTab(name, heb)
        ...
        btn.MouseButton1Click:Connect(function() 
            for _,v in pairs(Sidebar:GetChildren()) do 
                if v:IsA("TextButton") then Library:Tween(v, {BackgroundColor3 = Theme.Dark, TextColor3 = Color3.fromRGB(150,150,150)}) end 
            end
            for _,v in pairs(Container:GetChildren()) do v.Visible = false end
            Library:Tween(btn, {BackgroundColor3 = Theme.Gold, TextColor3 = Color3.new(0,0,0)})
            page.Visible = true 
        end)
    ...
    local Tab_Farm = CreateTab("Farming", "חווה")
    ...
    local function CreateSlider(parent, title, heb, min, max, default, callback, toggleCallback)
        local f = Instance.new("Frame", parent); f.Size = UDim2.new(0.95,0,0,75); f.BackgroundColor3 = Theme.Box; Library:Corner(f, 8); Library:AddGlow(f, Color3.fromRGB(40,40,40))
        ...
        local fill = Instance.new("Frame", line); fill.Size = UDim2.new((default-min)/(max-min),0,1,0); fill.BackgroundColor3 = Theme.Gold; Library:Corner(fill,6)
        ...
        if toggleCallback then 
            ...
            t.MouseButton1Click:Connect(function() 
                ...
                t.BackgroundColor3=on and Theme.Gold or Color3.fromRGB(40,40,40)
    ...
    local function CreateBigToggle(parent, title, heb, callback)
        local f = Instance.new("TextButton", parent); f.Size = UDim2.new(0.95,0,0,50); f.BackgroundColor3 = Theme.Box; f.Text=""; Library:Corner(f, 8); Library:AddGlow(f, Color3.fromRGB(40,40,40))
        ...
        f.MouseButton1Click:Connect(function() 
            on = not on
            Library:Tween(icon, {BackgroundColor3 = on and Theme.Gold or Color3.fromRGB(50,50,50)})
            callback(on) 
        end)
    end

    local function CreateSquareBind(parent, id, title, heb, default, callback)
        local f = Instance.new("TextButton", parent); f.Size = UDim2.new(id==3 and 1 or 0.48,0,0,80); f.BackgroundColor3 = Theme.Box; f.Text=""; Library:Corner(f, 8); Library:AddGlow(f, Color3.fromRGB(40,40,40))
        local k = Instance.new("TextLabel", f); k.Size = UDim2.new(1,0,0,30); k.Position = UDim2.new(0,0,0.5,0); k.Text=default.Name; k.TextColor3=Theme.Gold; k.Font=Enum.Font.GothamBold; k.TextSize=20; k.BackgroundTransparency=1
        f.MouseButton1Click:Connect(function() 
            k.Text="..."
            ...
            task.spawn(function() 
                while Settings.Farming do 
                    ...
                    if hrp and drops then 
                        for _, vk in pairs(drops:GetChildren()) do 
                            if vk:IsA("BasePart") and not FarmBlacklist[vk] then target = vk break end 
                        end 
                    end
    ...
    local function ToggleFly(v) 
        ...
        if v then 
            local bv = Instance.new("BodyVelocity",hrp); bv.Velocity=Vector3.zero; bv.MaxForce=Vector3.new(1e9,1e9,1e9); bv.Name="F_V"
            local bg = Instance.new("BodyGyro",hrp); bg.CFrame=hrp.CFrame; bg.MaxTorque=Vector3.new(1e9,1e9,1e9); bg.Name="F_G"
            task.spawn(function() 
    ...
    CreateSlider(Tab_Sett, "GUI Scale", "גודל", 0.5, 1.5, 1, function(v) MainScale.Scale = v end)
    
    local AddCr = function(n, id)
        local f = Instance.new("Frame", Tab_Cred); f.Size = UDim2.new(0.95,0,0,100); f.BackgroundColor3 = Theme.Box; Library:Corner(f, 12); Library:AddGlow(f)
        ...
        local t = Instance.new("TextLabel", f); t.Size = UDim2.new(0,350,0,30); t.Position = UDim2.new(0,100,0,20); t.Text = n .. " <font color='#AAAAAA'>(יוצרים)</font>"; t.RichText=true; t.TextColor3 = Theme.Gold; t.Font=Enum.Font.GothamBlack; t.TextSize=22; t.TextXAlignment="Left"; t.BackgroundTransparency=1
...
--// מסך מפתח - יצירה והפעלה
local KeyFrame = Instance.new("Frame", ScreenGui); KeyFrame.Size = UDim2.new(0, 400, 0, 260); KeyFrame.Position = UDim2.new(0.5, -200, 0.5, -130); KeyFrame.BackgroundColor3 = Theme.Dark; Library:Corner(KeyFrame, 10); Library:AddGlow(KeyFrame); Library:MakeDraggable(KeyFrame)
...
local KeyInput = Instance.new("TextBox", KeyFrame); KeyInput.Size = UDim2.new(0.7,0,0,45); KeyInput.Position = UDim2.new(0.15,0,0.4,0); KeyInput.BackgroundColor3 = Theme.Box; KeyInput.TextColor3 = Color3.new(1,1,1); KeyInput.PlaceholderText = "Enter Key..."; KeyInput.Text = ""; KeyInput.Font = Enum.Font.Gotham; KeyInput.TextSize = 14; Library:Corner(KeyInput, 6)
local KeyBtn = Instance.new("TextButton", KeyFrame); KeyBtn.Size = UDim2.new(0.4,0,0,40); KeyBtn.Position = UDim2.new(0.3,0,0.75,0); KeyBtn.BackgroundColor3 = Theme.Gold; KeyBtn.Text = "LOGIN / כניסה"; KeyBtn.Font = Enum.Font.GothamBold; KeyBtn.TextSize = 16; Library:Corner(KeyBtn, 6)
...
local function Authenticate(inputKey)
    ...
    if success and allKeysRaw and string.find(allKeysRaw, inputKey) then
        return true
    else
        warn("Key Auth Failed.")
        KeyBtn.Text = "INVALID KEY"; task.wait(2); KeyBtn.Text = "LOGIN / כניסה"; KeyBtn.Active = true
        return false
    end
end

KeyBtn.MouseButton1Click:Connect(function()
    if Authenticate(KeyInput.Text) then
...
print("[SYSTEM] Spaghetti Mafia Hub Ready.")
