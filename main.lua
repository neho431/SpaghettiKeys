...
local function CreateBigToggle(parent, title, heb, callback, toggleName)
    ...
    local on = false
    local function Update(s) on=s; if on then Library:Tween(icon,{BackgroundColor3=Settings.Theme.Gold}); Library:AddGlow(f,Settings.Theme.Gold) else Library:Tween(icon,{BackgroundColor3=Color3.fromRGB(50,50,50)}); if f:FindFirstChild("UIStroke") then f:FindFirstChild("UIStroke"):Destroy() end; Library:AddGlow(f,Color3.fromRGB(40,40,40)) end end
