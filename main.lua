--[[
    Spaghetti Mafia Hub v4 (FUN WORLD HEADER + SPINNING LOADING)
    Updates:
    - Loading: Compact Box, SPINNING Pasta 🍝, Hebrew Text.
    - Sidebar Header: Added "בוא לכיף" (Come for Fun) in Blue + Snow effect.
    - Main Header: Cleaned up (removed subtitle).
    - Logic: 100% Preserved.
]]
...
local Settings = {
    Theme = {
        ...
        IceBlue = Color3.fromRGB(100, 220, 255), -- כחול לקטגוריית חורף
...
--// 4. מסך טעינה - סגנון v1 אבל משופר (מסתובב)
...
LoadBox.Size = UDim2.new(0, 220, 0, 160) -- קומפקטי כמו בהתחלה
...
PastaIcon.AnchorPoint = Vector2.new(0.5, 0.5) -- מרכוז לסיבוב
...
-- אנימציית סיבוב פסטה + שלג
task.spawn(function()
    local rot = 0
    while LoadBox.Parent do
        rot = rot + 5
        PastaIcon.Rotation = rot -- מסתובב
        SpawnSnow(LoadBox, 2, 4) -- שלג יורד בתוך הקופסה
        task.wait(0.03)
    end
end)
...
-- אזור "בוא לכיף" - בראש ה-Sidebar
local FunHeader = Instance.new("Frame", Sidebar)
FunHeader.LayoutOrder = 0 -- תמיד ראשון
FunHeader.Size = UDim2.new(0.9, 0, 0, 50) -- גובה האזור
FunHeader.BackgroundTransparency = 1
FunHeader.ClipsDescendants = true -- שהשלג לא יברח החוצה

local FunText = Instance.new("TextLabel", FunHeader)
FunText.Size = UDim2.new(1, 0, 1, 0)
FunText.BackgroundTransparency = 1
FunText.Text = "בוא לכיף ❄️"
FunText.Font = Enum.Font.GothamBlack
FunText.TextSize = 18
FunText.TextColor3 = Settings.Theme.IceBlue -- כחול
FunText.ZIndex = 2

-- אפקט שלג רק בתוך ה"בוא לכיף"
task.spawn(function()
    while FunHeader.Parent do
        -- פונקציית שלג מותאמת ללוקאליות של ה-Header
        local flake = Instance.new("Frame", FunHeader)
        flake.Size = UDim2.new(0, math.random(2,4), 0, math.random(2,4))
        flake.Position = UDim2.new(math.random(10,90)/100, 0, -0.2, 0)
        flake.BackgroundColor3 = Color3.new(1,1,1)
        flake.BorderSizePixel = 0
        Library:Corner(flake, 10)
        TweenService:Create(flake, TweenInfo.new(math.random(2,4), Enum.EasingStyle.Linear), {
            Position = UDim2.new(flake.Position.X.Scale, 0, 1.2, 0),
            BackgroundTransparency = 1
        }):Play()
        Debris:AddItem(flake, 4)
        task.wait(0.3)
    end
end)
...
-- הקו הזז (בצד שמאל של הכפתורים)
local ActiveLine = Instance.new("Frame", Sidebar)
ActiveLine.Size = UDim2.new(0, 3, 0, 45)
...
local function CreateTab(name, heb, order, isWinter)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        -- חישוב מיקום יחסי ל-Sidebar כדי להתעלם מה-Header
        Library:Tween(ActiveLine, {Position = UDim2.new(0, 0, 0, btn.AbsolutePosition.Y - Sidebar.AbsolutePosition.Y)}, 0.3)
    end)
...
print("[SYSTEM] Spaghetti Mafia Hub v4 (Final) Loaded")
