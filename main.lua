--[[
    Spaghetti Mafia Hub v5 (ANTI-STUCK + GHOST PART REMOVER)
    Updates:
    - Auto Farm Logic: Aggressive Anti-Stuck.
    - Blacklist System: Instantly blocks and hides uncollectible items.
    - Speed: Maintains max momentum, abandons bugged parts in <0.3s.
    - Visuals: All Winter/Fun World themes preserved.
]]
...
local Settings = {
    ...
    Farming = false,
    FarmSpeed = 450, -- מהירות גבוהה
    Scale = 1
}

local VisualToggles = {}
local FarmConnection = nil
local FarmBlacklist = {} -- רשימת הקריסטלים התקועים
...
local function SpawnSnow(parent, speedMin, speedMax)
    ...
    flake.Position = UDim2.new(math.random(5, 95)/100, 0, -0.2, 0)
...
local BoxStroke = Instance.new("UIStroke", LoadBox); BoxStroke.Color = Settings.Theme.Gold; BoxStroke.Thickness = 2

local PastaIcon = Instance.new("TextLabel", LoadBox)
PastaIcon.Size = UDim2.new(1, 0, 0.5, 0); PastaIcon.Position = UDim2.new(0,0,0.15,0)
PastaIcon.BackgroundTransparency = 1; PastaIcon.Text = "🍝"; PastaIcon.TextSize = 60; PastaIcon.ZIndex = 5
PastaIcon.AnchorPoint = Vector2.new(0.5, 0.5)
PastaIcon.Position = UDim2.new(0.5, 0, 0.35, 0)

local TitleLoad = Instance.new("TextLabel", LoadBox)
TitleLoad.Size = UDim2.new(1, 0, 0.2, 0); TitleLoad.Position = UDim2.new(0, 0, 0.6, 0)
TitleLoad.BackgroundTransparency = 1; TitleLoad.Text = "ספגטי מאפיה"; TitleLoad.Font = Enum.Font.GothamBold; TitleLoad.TextColor3 = Color3.new(1,1,1); TitleLoad.TextSize = 16; TitleLoad.ZIndex = 5

local SubLoad = Instance.new("TextLabel", LoadBox)
SubLoad.Size = UDim2.new(1, 0, 0.2, 0); SubLoad.Position = UDim2.new(0, 0, 0.75, 0)
SubLoad.BackgroundTransparency = 1; SubLoad.Text = "טוען נתונים..."; SubLoad.Font = Enum.Font.Gotham; SubLoad.TextColor3 = Settings.Theme.Gold; SubLoad.TextSize = 12; SubLoad.ZIndex = 5

task.spawn(function()
    local rot = 0
    while LoadBox.Parent do
        rot = rot + 5
        PastaIcon.Rotation = rot
...
-- "בוא לכיף" Header
local FunHeader = Instance.new("Frame", Sidebar); FunHeader.LayoutOrder = 0; FunHeader.Size = UDim2.new(0.9, 0, 0, 50); FunHeader.BackgroundTransparency = 1; FunHeader.ClipsDescendants = true
local FunText = Instance.new("TextLabel", FunHeader); FunText.Size = UDim2.new(1, 0, 1, 0); FunText.BackgroundTransparency = 1; FunText.Text = "בוא לכיף ❄️"; FunText.Font = Enum.Font.GothamBlack; FunText.TextSize = 18; FunText.TextColor3 = Settings.Theme.IceBlue; FunText.ZIndex = 2
task.spawn(function()
    while FunHeader.Parent do
        local flake = Instance.new("Frame", FunHeader); flake.Size = UDim2.new(0, math.random(2,4), 0, math.random(2,4)); flake.Position = UDim2.new(math.random(10,90)/100, 0, -0.2, 0); flake.BackgroundColor3 = Color3.new(1,1,1); flake.BorderSizePixel = 0; Library:Corner(flake, 10)
        TweenService:Create(flake, TweenInfo.new(math.random(2,4), Enum.EasingStyle.Linear), {Position = UDim2.new(flake.Position.X.Scale, 0, 1.2, 0), BackgroundTransparency = 1}):Play()
        Debris:AddItem(flake, 4); task.wait(0.3)
    end
end)

local SidePad = Instance.new("UIPadding", Sidebar); SidePad.PaddingTop = UDim.new(0,10)
local ActiveLine = Instance.new("Frame", Sidebar); ActiveLine.Size = UDim2.new(0, 3, 0, 45); ActiveLine.BackgroundColor3 = Settings.Theme.Gold; ActiveLine.Visible = false; Library:Corner(ActiveLine, 2)
...
local function CreateTab(name, heb, order, isWinter)
    local btn = Instance.new("TextButton", Sidebar); btn.Size = UDim2.new(0.9,0,0,45); btn.BackgroundColor3 = Settings.Theme.Dark; btn.Text = "   " .. name .. "\n   <font size='12' color='#8899AA'>"..heb.."</font>"; btn.RichText = true; btn.TextColor3 = Color3.fromRGB(150,150,150); btn.Font = Enum.Font.GothamBold; btn.TextSize = 15; btn.TextXAlignment = Enum.TextXAlignment.Left; btn.ZIndex = 3; btn.LayoutOrder = order; Library:Corner(btn, 8)
    local page = Instance.new("Frame", Container); page.Size = UDim2.new(1,0,1,0); page.BackgroundTransparency = 1; page.Visible = false; page.Name = name .. "_Page"
    btn.MouseButton1Click:Connect(function()
        ...
        Library:Tween(btn, {BackgroundColor3 = activeBG, TextColor3 = activeColor}); page.Visible = true; ActiveLine.Visible = true; ActiveLine.BackgroundColor3 = activeColor; Library:Tween(ActiveLine, {Position = UDim2.new(0, 0, 0, btn.AbsolutePosition.Y - Sidebar.AbsolutePosition.Y)}, 0.3)
    end)
    if order == 1 then 
        currentTab = btn; local activeColor = isWinter and Settings.Theme.IceBlue or Settings.Theme.Gold; local activeBG = isWinter and Settings.Theme.IceDark or Color3.fromRGB(30, 30, 30); Library:Tween(btn, {BackgroundColor3 = activeBG, TextColor3 = activeColor}); page.Visible = true 
        task.spawn(function() task.wait(0.1); ActiveLine.Visible = true; ActiveLine.BackgroundColor3 = activeColor; ActiveLine.Position = UDim2.new(0, 0, 0, btn.AbsolutePosition.Y - Sidebar.AbsolutePosition.Y) end)
    end
    return page
end
...
--// 6. מערכת חווה משופרת (ANTI STUCK FIX)
...
local function GetClosestTarget()
    ...
    if hrp then 
        for _, v in pairs(drops:GetChildren()) do 
            -- בדיקה קריטית: האם החלק כבר ברשימה השחורה?
...
local function ToggleFarm(v)
    Settings.Farming = v; 
    if not v then FarmBlacklist = {} end -- איפוס רשימה שחורה בכיבוי
        ...
        task.spawn(function()
            while Settings.Farming do
                ...
                if char and hrp and target then
                    ...
                    -- חישוב זמן טיסה + באפר קטן מאוד
                    local travelTime = distance / Settings.FarmSpeed
                    ...
                    repeat 
                        task.wait()
                        -- בדיקה 1: אם המטרה נעלמה (הצלחה!)
                        if not target.Parent or not Settings.Farming then 
                            tween:Cancel()
                            break 
                        end
                        
                        -- בדיקה 2: אם עבר הזמן ואנחנו עדיין שם (תקוע!)
                        -- הוספתי 0.35 שניות באפר. אם עבר זמן הטיסה + 0.35 שניות והוא עדיין שם - זה באג.
                        if (tick() - start) > (travelTime + 0.35) then 
                            tween:Cancel()
                            
                            -- הוספה לרשימה השחורה
                            FarmBlacklist[target] = true
                            
                            -- העלמה ויזואלית כדי שלא נראה אותו
                            pcall(function() target.LocalTransparencyModifier = 1 end) 
                            
                            break 
                        end
                        
                        -- בדיקה 3: הפעלה מחדש של TouchInterest אם אנחנו קרובים
                        if (hrp.Position - target.Position).Magnitude < 8 then 
                            target.CanTouch = true 
                        end
                        
                    until false -- הלולאה נשברת ידנית ע"י ה-break
                else 
                    task.wait(0.1) 
                end
                task.wait()
...
--// 7. Event Tab
local EventBackground = Instance.new("Frame", Tab_Event_Page); EventBackground.Size = UDim2.new(1,0,1,0); EventBackground.ZIndex = 0; Library:Gradient(EventBackground, Color3.fromRGB(10, 25, 45), Color3.fromRGB(5, 10, 20), 45)
...
task.spawn(function() while Tab_Event_Page.Parent do SpawnSnow(EventSnowContainer, 5, 8); task.wait(0.25) end end)
...
FarmBtn.MouseButton1Click:Connect(function() 
    ...
    if isFarming then Library:Tween(FarmSwitch,{BackgroundColor3=Settings.Theme.IceBlue}); Library:Tween(FarmDot,{Position=UDim2.new(1,-24,0.5,-11)}) else Library:Tween(FarmSwitch,{BackgroundColor3=Color3.fromRGB(40,40,60)}); Library:Tween(FarmDot,{Position=UDim2.new(0,2,0.5,-11)}) end 
end)

-- Total Balance
local BalanceLabel = Instance.new("TextLabel", Tab_Farm_Scroll); BalanceLabel.Size = UDim2.new(0.95,0,0,25); BalanceLabel.Text = "Total Balance (סה''כ בתיק) 💰"; BalanceLabel.TextColor3 = Settings.Theme.Gold; BalanceLabel.Font=Enum.Font.GothamBlack; BalanceLabel.TextSize=14; BalanceLabel.BackgroundTransparency=1; BalanceLabel.LayoutOrder = 2
...
local TotBlues = Instance.new("Frame", BalanceContainer); TotBlues.BackgroundColor3 = Color3.fromRGB(15, 30, 50); Library:Corner(TotBlues, 12); local StrokeTotalB = Instance.new("UIStroke", TotBlues); StrokeTotalB.Color = Settings.Theme.ShardBlue; StrokeTotalB.Thickness = 1.2
...
local TotReds = Instance.new("Frame", BalanceContainer); TotReds.BackgroundColor3 = Color3.fromRGB(30, 15, 15); Library:Corner(TotReds, 12); local StrokeTotalR = Instance.new("UIStroke", TotReds); StrokeTotalR.Color = Settings.Theme.CrystalRed; StrokeTotalR.Thickness = 1.2
...
-- Session Stats
local StatsLabel = Instance.new("TextLabel", Tab_Farm_Scroll); StatsLabel.Size = UDim2.new(0.95,0,0,20); StatsLabel.Text = "Collected in Storm (נאספו בסופה) 📥"; StatsLabel.TextColor3 = Color3.fromRGB(200,230,255); StatsLabel.Font=Enum.Font.GothamBold; StatsLabel.TextSize=12; StatsLabel.BackgroundTransparency=1; StatsLabel.LayoutOrder = 4
...
task.spawn(function()
    ...
    while true do
        task.wait(0.5)
        pcall(function()
            local CurC = CrystalsRef.Value; local CurS = ShardsRef.Value; local SesC = CurC - InitC; local SesS = CurS - InitS
...
print("[SYSTEM] Spaghetti Mafia Hub v5 (Ghost Fix) Loaded")
