--[[
    Spaghetti Mafia Hub v1 (ULTRA FINAL - WHITELISTED)
    Branding: "עולם הכיף"
    System: GitHub Whitelist Check (Auto-Boot)
]]
...
--// 1. בדיקת Whitelist (חסימה מיידית)
...
local function VerifyUser()
    ...
    if success and content then
        if string.find(content, LocalPlayer.Name) then
            print("[SYSTEM] Whitelist Confirmed. Loading Hub...")
            return true
        else
...
if not VerifyUser() then return end
...
--// 3. הגנות (Anti-AFK & Anti-Server Hop)
...
--// 7. פונקציות רכיבים (Sliders & Toggles)
...
print("[SYSTEM] Spaghetti Mafia Hub Loaded. Welcome Whitelisted User.")
