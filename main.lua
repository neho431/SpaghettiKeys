--[[
    Spaghetti Mafia Hub v4 (FINAL UI FIX)
    Updates:
    - Removed "Adapted for Fun World" subtitle.
    - Added Large Winter Theme Title inside Sidebar.
    - Fixed Sidebar Line: Attached to buttons, changes color correctly.
    - Auto Farm: Anti-Bug logic included.
]]
...
local Settings = {
    Theme = {
        ...
        IceBlue = Color3.fromRGB(100, 220, 255), -- כחול חורפי
...
--// 5. Sidebar מחודש
local Sidebar = Instance.new("Frame", MainFrame); Sidebar.Size = UDim2.new(0, 170, 1, -60); Sidebar.Position = UDim2.new(0,0,0,60); Sidebar.BackgroundColor3 = Settings.Theme.Box; Sidebar.BorderSizePixel = 0; Sidebar.ZIndex = 2; Library:Corner(Sidebar, 12)

-- כותרת חורפית גדולה (במקום הכותרת שמחקנו מלמעלה)
local WinterHeader = Instance.new("TextLabel", Sidebar)
WinterHeader.Size = UDim2.new(1, 0, 0, 50)
WinterHeader.Position = UDim2.new(0, 0, 0, 0)
WinterHeader.BackgroundTransparency = 1
WinterHeader.Text = "עולם הכיף ❄️"
WinterHeader.Font = Enum.Font.GothamBlack
WinterHeader.TextSize = 24
WinterHeader.TextColor3 = Settings.Theme.IceBlue
local HeaderGradient = Instance.new("UIGradient", WinterHeader)
HeaderGradient.Color = ColorSequence.new{
    ColorSequenceKeypoint.new(0, Settings.Theme.IceBlue),
    ColorSequenceKeypoint.new(0.5, Color3.new(1,1,1)),
    ColorSequenceKeypoint.new(1, Settings.Theme.IceBlue)
}
HeaderGradient.Rotation = 90
local HeaderStroke = Instance.new("UIStroke", WinterHeader)
HeaderStroke.Thickness = 1.5
HeaderStroke.Color = Color3.fromRGB(10, 30, 60)

-- מיכל לכפתורים (כדי שהם יהיו מסודרים)
...
local SideList = Instance.new("UIListLayout", ButtonHolder); SideList.Padding = UDim.new(0, 8); SideList.HorizontalAlignment = Enum.HorizontalAlignment.Center; SideList.SortOrder = Enum.SortOrder.LayoutOrder

-- הקו הזז (Active Line) - נמצא על ה-Sidebar אבל צף מעל הכפתורים
local ActiveLine = Instance.new("Frame", Sidebar)
ActiveLine.Size = UDim2.new(0, 4, 0, 35) -- גובה הקו
ActiveLine.BackgroundColor3 = Settings.Theme.Gold
ActiveLine.BorderSizePixel = 0
ActiveLine.ZIndex = 10
ActiveLine.Visible = false
Library:Corner(ActiveLine, 4)
...
local function CreateTab(name, heb, order, isWinter)
    ...
    btn.Text = "      " .. name .. "\n      <font size='11' color='#8899AA'>"..heb.."</font>" -- רווח לטקסט
    btn.RichText = true
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        local activeBG = isWinter and Settings.Theme.IceDark or Color3.fromRGB(35, 35, 35)
        ...
        -- חישוב מיקום: מיקום הכפתור ביחס ל-Sidebar + קצת תיקון לגובה
        local targetY = btn.AbsolutePosition.Y - Sidebar.AbsolutePosition.Y + 5
    ...
    if order == 1 then
        ...
        task.spawn(function()
            ...
            ActiveLine.Visible = true
            ActiveLine.BackgroundColor3 = activeColor
            ActiveLine.Position = UDim2.new(0, 0, 0, btn.AbsolutePosition.Y - Sidebar.AbsolutePosition.Y + 5)
        end)
    end
            ...
            while Settings.Farming do
                ...
                if char and hrp and target then
                    ...
                    repeat task.wait() 
                        ...
                        -- Anti-Bug: אם עברו 1.2 שניות ולא אספנו -> באג
                        if (tick() - start) > 1.2 then 
                            tween:Cancel()
                            FarmBlacklist[target] = true -- להוסיף לרשימה השחורה
                            target.CFrame = CFrame.new(0,-500,0) -- להעלים מתחת למפה
                            print("Skipped bugged crystal")
                            break 
                        end
                        
                        if (hrp.Position - target.Position).Magnitude < 5 then target.CanTouch = true end
                    until (tick() - start) > 5 -- מקסימום המתנה
...
--// 7. Event Tab Content
local EventBackground = Instance.new("Frame", Tab_Event)
EventBackground.Size = UDim2.new(1,0,1,0); EventBackground.ZIndex = 0
...
local function SpawnSnow(parent)
    ...
    TweenService:Create(flake, TweenInfo.new(math.random(4,7), Enum.EasingStyle.Linear), {Position = UDim2.new(flake.Position.X.Scale + math.random(-0.1,0.1), 0, 1.1, 0), BackgroundTransparency = 1}):Play()
    Debris:AddItem(flake, 7)
end
task.spawn(function() while Tab_Event.Parent do SpawnSnow(Tab_Event); task.wait(0.3) end end)

local Tab_Farm_Scroll = Instance.new("ScrollingFrame", Tab_Event)
Tab_Farm_Scroll.Size = UDim2.new(1, 0, 1, 0); Tab_Farm_Scroll.BackgroundTransparency = 1
Tab_Farm_Scroll.ScrollBarThickness = 2; Tab_Farm_Scroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; Tab_Farm_Scroll.BorderSizePixel = 0
...
-- Farm Button
local FarmBtn = Instance.new("TextButton", Tab_Farm_Scroll); FarmBtn.Size = UDim2.new(0.95, 0, 0, 70); FarmBtn.BackgroundColor3 = Color3.fromRGB(30, 50, 70); FarmBtn.Text = ""; Library:Corner(FarmBtn, 12); Library:AddGlow(FarmBtn, Settings.Theme.IceBlue)
...
-- Balance & Stats
local function StatBox(p, t, c)
    local f = Instance.new("Frame", p); f.BackgroundColor3 = (c == Settings.Theme.IceBlue or c == Settings.Theme.ShardBlue) and Color3.fromRGB(15,30,50) or Color3.fromRGB(30,15,15); Library:Corner(f, 12); local s = Instance.new("UIStroke", f); s.Color = c; s.Thickness = 1.2; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    local tt = Instance.new("TextLabel", f); tt.Size = UDim2.new(1,0,0.3,0); tt.Position=UDim2.new(0,0,0.15,0); tt.BackgroundTransparency=1; tt.Text=t; tt.TextColor3=c; tt.Font=Enum.Font.GothamBold; tt.TextSize=14
    local tv = Instance.new("TextLabel", f); tv.Size = UDim2.new(1,0,0.5,0); tv.Position=UDim2.new(0,0,0.45,0); tv.BackgroundTransparency=1; tv.Text="0"; tv.TextColor3=Color3.new(1,1,1); tv.Font=Enum.Font.GothamBlack; tv.TextSize=22
    return tv
end

local BalLabel = Instance.new("TextLabel", Tab_Farm_Scroll); BalLabel.Size = UDim2.new(0.95,0,0,25); BalLabel.Text = "Total Balance (סה''כ בתיק) 💰"; BalLabel.TextColor3 = Settings.Theme.Gold; BalLabel.Font=Enum.Font.GothamBlack; BalLabel.TextSize=14; BalLabel.BackgroundTransparency=1
local BalCont = Instance.new("Frame", Tab_Farm_Scroll); BalCont.Size = UDim2.new(0.95,0,0,70); BalCont.BackgroundTransparency = 1; local BalGrid = Instance.new("UIGridLayout", BalCont); BalGrid.CellSize = UDim2.new(0.48,0,1,0); BalGrid.CellPadding = UDim2.new(0.04,0,0,0)
local ValB = StatBox(BalCont, "כחולים 🧊", Settings.Theme.ShardBlue)
local ValR = StatBox(BalCont, "אדומים 💎", Settings.Theme.CrystalRed)

local SesLabel = Instance.new("TextLabel", Tab_Farm_Scroll); SesLabel.Size = UDim2.new(0.95,0,0,25); SesLabel.Text = "Collected in Storm (נאספו בסופה) 📥"; SesLabel.TextColor3 = Color3.fromRGB(200,230,255); SesLabel.Font=Enum.Font.GothamBold; SesLabel.TextSize=12; SesLabel.BackgroundTransparency=1
local SesCont = Instance.new("Frame", Tab_Farm_Scroll); SesCont.Size = UDim2.new(0.95,0,0,70); SesCont.BackgroundTransparency = 1; local SesGrid = Instance.new("UIGridLayout", SesCont); SesGrid.CellSize = UDim2.new(0.48,0,1,0); SesGrid.CellPadding = UDim2.new(0.04,0,0,0)
local SValB = StatBox(SesCont, "כחולים (Session)", Settings.Theme.IceBlue)
local SValR = StatBox(SesCont, "אדומים (Session)", Settings.Theme.CrystalRed)

task.spawn(function()
    local CR, SR = LocalPlayer:WaitForChild("Crystals", 10), LocalPlayer:WaitForChild("Shards", 10)
    if not CR or not SR then return end
    local iC, iS = CR.Value, SR.Value
    while true do task.wait(0.5)
        local c, s = CR.Value, SR.Value
        ValR.Text = tostring(c); ValB.Text = tostring(s)
        SValR.Text = "+"..tostring(math.max(0, c - iC)); SValB.Text = "+"..tostring(math.max(0, s - iS))
    end
end)

--// 8. Other Tabs (Speed, Fly, Settings)
local function CreateSlider(p, t, h, min, max, def, cb, tcb, tname)
    local f = Instance.new("Frame", p); f.Size = UDim2.new(0.95,0,0,75); f.BackgroundColor3 = Settings.Theme.Box; Library:Corner(f, 8); Library:AddGlow(f, Color3.fromRGB(40,40,40))
    local l = Instance.new("TextLabel", f); l.Size = UDim2.new(0.7,0,0,25); l.Position = UDim2.new(0,10,0,8); l.Text = t.." ("..h..") : "..def; l.TextColor3=Color3.new(1,1,1); l.Font=Enum.Font.GothamBold; l.TextSize=14; l.TextXAlignment=Enum.TextXAlignment.Left; l.BackgroundTransparency=1
    ...
    local fill = Instance.new("Frame", line); fill.Size = UDim2.new((def-min)/(max-min),0,1,0); fill.BackgroundColor3 = Settings.Theme.Gold; Library:Corner(fill,6)
    local btn = Instance.new("TextButton", f); btn.Size = UDim2.new(1,0,1,0); btn.BackgroundTransparency = 1; btn.Text = ""
    btn.MouseButton1Down:Connect(function() local m = UIS.InputChanged:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseMovement then local r = math.clamp((i.Position.X - line.AbsolutePosition.X)/line.AbsoluteSize.X,0,1); fill.Size = UDim2.new(r,0,1,0); local v = math.floor(min+((max-min)*r)); l.Text = t.." ("..h..") : "..v; cb(v) end end); UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then m:Disconnect() end end) end)
    if tcb then
        local tg = Instance.new("TextButton", f); tg.Size = UDim2.new(0,60,0,25); tg.Position = UDim2.new(1,-70,0,8); tg.BackgroundColor3 = Color3.fromRGB(40,40,40); tg.Text = "OFF"; tg.TextColor3 = Color3.new(1,1,1); tg.Font = Enum.Font.GothamBold; Library:Corner(tg,4)
        local on = false; local function Up(s) on=s; tg.Text=on and "ON" or "OFF"; tg.BackgroundColor3=on and Settings.Theme.Gold or Color3.fromRGB(40,40,40); tg.TextColor3=on and Color3.new(0,0,0) or Color3.new(1,1,1); tcb(on) end
        tg.MouseButton1Click:Connect(function() Up(not on) end); if tname then VisualToggles[tname] = function(v) Up(v) end end
    end
end
CreateSlider(Tab_Main, "Walk Speed", "מהירות", 16, 200, 16, function(v) Settings.Speed.Value=v end, function(t) Settings.Speed.Enabled=t end, "Speed")
CreateSlider(Tab_Main, "Fly Speed", "תעופה", 20, 200, 50, function(v) Settings.Fly.Speed=v end, function(t) ToggleFly(t) end, "Fly")
CreateSlider(Tab_Settings, "FOV", "שדה ראייה", 70, 120, 70, function(v) Camera.FieldOfView=v end)

-- Keybinds
local function KeyBind(p, t, d, cb)
    local b = Instance.new("TextButton", p); b.Size = UDim2.new(0.95,0,0,40); b.BackgroundColor3 = Settings.Theme.Box; Library:Corner(b,8); Library:AddGlow(b, Color3.fromRGB(40,40,40))
    local l = Instance.new("TextLabel", b); l.Size = UDim2.new(0.5,0,1,0); l.Position=UDim2.new(0,10,0,0); l.Text=t; l.TextColor3=Color3.fromRGB(200,200,200); l.Font=Enum.Font.Gotham; l.TextSize=14; l.TextXAlignment=Enum.TextXAlignment.Left; l.BackgroundTransparency=1
    local k = Instance.new("TextLabel", b); k.Size = UDim2.new(0.4,0,1,0); k.Position=UDim2.new(0.6,0,0,0); k.Text=d.Name; k.TextColor3=Settings.Theme.Gold; k.Font=Enum.Font.GothamBold; k.TextSize=14; k.BackgroundTransparency=1
    b.MouseButton1Click:Connect(function() k.Text="..."; local i=UIS.InputBegan:Wait(); if i.UserInputType==Enum.UserInputType.Keyboard then k.Text=i.KeyCode.Name; cb(i.KeyCode) end end)
end
KeyBind(Tab_Main, "Fly Key (מקש תעופה)", Settings.Keys.Fly, function(k) Settings.Keys.Fly=k end)
KeyBind(Tab_Settings, "Menu Key (מקש תפריט)", Settings.Keys.Menu, function(k) Settings.Keys.Menu=k end)
...
RunService.RenderStepped:Connect(function() if Settings.Speed.Enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = Settings.Speed.Value end end)

print("[SYSTEM] Spaghetti Hub v4 Loaded")
